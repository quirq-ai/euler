#!/usr/bin/env bash
# Euler local runner and process manager
# Usage: ./euler.sh {dev|install|start|stop|restart|status|logs}
#
# The /tmp basenames follow the same shape xo-space uses for its process
# (/tmp/euler.{pid,log}), so one name finds it everywhere.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ENV_FILE="$SCRIPT_DIR/.env"
PID_FILE="/tmp/euler.pid"
LOG_FILE="/tmp/euler.log"
LOCK_FILE="/tmp/euler.lock"
...
acquire_lock() {
    ...
    log_error "Another euler.sh operation is in progress"
    exit 1
}
...
select_local_port() {
    local preferred_port="${1:-2718}"

    if [ "$preferred_port" != "2718" ]; then
    ...
    if ! port_is_in_use "2718"; then
        printf '2718\n'
        return 0
    fi

    if ! port_is_in_use "2719"; then
        log_warn "Local port 2718 is already in use; using 2719" >&2
        printf '2719\n'
        return 0
    fi

    log_error "Local ports 2718 and 2719 are both in use; set PORT to another port" >&2
    return 1
}
...
find_orphan_server_pids() {
    ...
    # Scoped to THIS checkout's interpreter and server.py. A bare
    # "python.*server.py" pattern matches every server.py on the machine
    # (other projects, other worktrees) and would kill them all.
    local candidates
    candidates=$(pgrep -f "^${SCRIPT_DIR}/venv/bin/python server\.py" 2>/dev/null | sort -u || true)
...
start_api() {
    acquire_lock
    if is_running; then
        echo -e "${YELLOW}Euler is already running (PID: $(cat "$PID_FILE"))${NC}"
        return 0
    fi
    ...
    log "Starting Euler on ${HOST}:${PORT}..."
    nohup bash -c '
        cd "'"$SCRIPT_DIR"'"
        export HOST="'"$HOST"'"
        export PORT="'"$PORT"'"
    ...
    if kill -0 "$pid" 2>/dev/null; then
        log_success "Euler started (PID: $pid, port: $PORT)"
        echo -e "Logs: ${CYAN}$LOG_FILE${NC}"
    else
        rm -f "$PID_FILE"
        log_error "Failed to start Euler. Check logs: $LOG_FILE"
        return 1
    fi
}
...
status_api() {
    if is_running; then
        echo -e "${GREEN}Euler is running${NC} (PID: $(cat "$PID_FILE"))"
    else
        echo -e "${RED}Euler is not running${NC}"
    fi
...
run_dev() {
    ...
    local preferred_port="${CONFIGURED_PORT:-2718}"
    local dev_port
    local python_cmd
    ...
    log "Starting development server at http://${dev_host}:${dev_port}"
